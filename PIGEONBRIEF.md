...
**Phase 1 Step 1~6 완료 (2026-04-09)**, **Step 8 CEO 베타 테스트 + 위저드 개선 완료 (2026-04-14)**
- 상세 계획: `docs/phase1_plan.md`
- 커밋: `7b30402` (Phase 1 초기), `6fe847c` (CEO 피드백 1차), `d916d54` (CEO 피드백 2차)
- **남은 작업**: 지인 5~10명 베타 확대 → 피드백 수집 → Phase 2 착수 판단

---

## 주요 변경 이력

### 2026-04-14 — Phase 1 Step 8: CEO 베타 테스트 피드백 반영 (위저드 개선 2라운드)

커밋 `6fe847c`, `d916d54` — `website/assets/app.js`, `website/assets/style.css` 수정.

**배경**: CEO가 pigeonbrief.com에서 위저드를 직접 사용한 후 수집한 피드백을 2라운드에 걸쳐 반영.

#### 1차 개선 (커밋 `6fe847c`) — 버그 수정 3건 + UX 개선 5건

**버그 수정**
- `pbApi()`: Clerk 세션 만료 시 `window.Clerk.session` null → "페이지를 새로고침해 주세요" 안내. 서버 4xx/5xx 응답도 JSON `detail` 파싱해서 구체적 메시지 표시 ("Failed to fetch" 원인 불명 문제 해결)
- 미리보기 결과 미표시: `preview.py`가 반환하는 상태(`empty`/`partial`/`no_llm`/`error`)별로 다른 배너 메시지 표시. 기사가 없을 때도 "이렇게 구독해볼까요?" 화면을 명확히 구분
- 저장 실패 "Failed to fetch": pbApi 에러 처리 개선으로 동일 케이스 해소

**UX 개선**
- Step 1: `관심 주제` 단일 필드 → **`섹션 이름`** (탭에 표시될 짧은 이름) + **`주제 설명`** (AI에게 전달) 두 필드로 분리. 이름 어색함 문제 해소
- Step 1: **수집 목적** 빠른 선택 버튼 5종 추가 (경쟁사 모니터링 / 시장 조사 / 기술 트렌드 / 투자 리서치 / 개인 관심). 선택된 목적은 AI 주제 해석 프롬프트에 포함
- Step 3: 핵심 키워드 칩에 **× 삭제** 버튼 + 직접 입력·추가 필드
- Step 3: "연관 고유명사 / 연관 개념" → **"참고 키워드"** 로 통합. "클릭하면 핵심 키워드로 추가" hint + 클릭 시 실제로 핵심 키워드 배열에 이동
- Step 3: 제외 키워드 **× 삭제** + 직접 추가 입력 필드

#### 2차 개선 (커밋 `d916d54`) — UX 3건

- **주제 설명 구조 가이드**: textarea 위에 `나는 누구` + `무슨 목적으로` + `무엇을 알고 싶어` 태그 배지 표시. placeholder도 실제 예시 문장으로 교체
- **수집 목적 버튼 중복 선택**: 복수 목적 동시 선택 허용 (`wizState.purposes: Set`). 선택된 목적 전체가 AI 호출 시 함께 전달
- **"AI에게 수정 요청" 제거, "추가 고려 사항"으로 대체**: 별도 수정 버튼 삭제. "미리보기 →" 클릭 시 입력값이 있으면 `/api/ai/edit-keywords` 먼저 호출 → 업데이트된 키워드셋으로 preview 실행 (실패 시 기존 키워드로 silent fallback)
